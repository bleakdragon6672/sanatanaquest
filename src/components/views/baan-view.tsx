...
export function BajrangBaanView() {
  ...
  if (selectedVerse) {
    return <VerseReader verse={selectedVerse} onBack={() => navigate('baan')} initialBookMode={params.bookMode === '1'} />
  }

  return <FullBaan initialBookMode={params.bookMode === '1'} />
}

function FullBaan({ initialBookMode = false }: { initialBookMode?: boolean }) {
...
function VerseReader({ verse, onBack, initialBookMode = false }: { verse: BaanVerse; onBack: () => void; initialBookMode?: boolean }) {
  const store = useStore()
  const { navigate } = useNav()
