    ...
    <>
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-2">
          {prevVerse ? <Button variant="outline" onClick={() => navigate('tandav', { verse: prevVerse.id })}><ChevronLeft className="mr-1 h-4 w-4" /> Stanza {prevVerse.number}</Button> : <span />}
          {nextVerse ? <Button variant="outline" onClick={() => navigate('tandav', { verse: nextVerse.id })}>Stanza {nextVerse.number} <ChevronRight className="ml-1 h-4 w-4" /></Button> : <span />}
        </div>
      </div>
