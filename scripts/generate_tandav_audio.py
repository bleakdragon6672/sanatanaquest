#!/usr/bin/env python3
"""Generate Sanskrit audio for Shiv Tandav Stotram (16 stanzas) using Edge TTS."""
import asyncio
import os
import re

def extract_tandav_verses(ts_path):
    with open(ts_path, 'r') as f:
        content = f.read()
    pattern = re.compile(r"id:\s*`(t\d+)`[^}]*?sanskrit:\s*`([^`]+)`", re.DOTALL)
    verses = []
...
async def main():
    import edge_tts
    ts_path = '/Users/samarthrao/prodgita/src/lib/shiv-tandav-data.ts'
    output_dir = '/Users/samarthrao/prodgita/public/audio/tandav'
    voice = 'hi-IN-MadhurNeural'
    rate = '-15%'
    
    os.makedirs(output_dir, exist_ok=True)
    verses = extract_tandav_verses(ts_path)
    print(f"Found {len(verses)} Tandav stanzas to generate")
